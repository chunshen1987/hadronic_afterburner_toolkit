...
for ipart in pair_list:
    ...
    file_name     = 'Correlation_function_%s_os_2D.dat' % ipart
    file_name_err = 'Correlation_function_%s_os_2D_err.dat' % ipart
    event_avg_data = loadtxt(
            path.join(file_folder_list[0],
                      'Correlation_function_9998_-9998_os_2D.dat'))*0.0
    event_avg_data_err = 0.0*event_avg_data
    for ifolder in range(nev):
        results_folder = path.abspath(file_folder_list[ifolder])
        print("processing %s/%s ..." % (results_folder, file_name))
    ...
    file_name     = 'Correlation_function_%s_ss_2D.dat' % ipart
    file_name_err = 'Correlation_function_%s_ss_2D_err.dat' % ipart
    event_avg_data = loadtxt(
            path.join(file_folder_list[0],
                      'Correlation_function_9998_-9998_ss_2D.dat'))*0.0
    event_avg_data_err = 0.0*event_avg_data
    for ifolder in range(nev):
        results_folder = path.abspath(file_folder_list[ifolder])
        print("processing %s/%s ..." % (results_folder, file_name))
