    ...
    do 
        ...
        if [ "$hydrostatus" == "Finished." ]; then
            if [ -a $eventsPath/$iev/$spvn_folder_name$event_id/particle_9999_vndata_eta_-0.5_0.5.dat ]; then
                ...
                mv $target_hydro_folder/$hydro_folder_name$event_id/eccentricities_evo_ed_tau_0.55.dat $target_spvn_folder/$spvn_folder_name$event_id
                mv $target_hydro_folder/$hydro_folder_name$event_id/eccentricities_evo_ed_tau_0.6.dat $target_spvn_folder/$spvn_folder_name$event_id
                mv $target_hydro_folder/$hydro_folder_name$event_id/eccentricities_evo_ed_tau_0.65.dat $target_spvn_folder/$spvn_folder_name$event_id
